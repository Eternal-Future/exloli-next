...
use exloli_next::onebot::{start_onebot, OneBotHub};
...
async fn main() -> Result<()> {
    ...
    let bot = Bot::new(&config.telegram.token)
        ...
        .cache_me();
    let onebot = OneBotHub::new(config.onebot.clone(), trans.clone());
    let uploader = ExloliUploader::new(
        config.clone(),
        ehentai.clone(),
        bot.clone(),
        trans.clone(),
    ...
    let t2 = {
        ...
        tokio::spawn(async move { start_dispatcher(dispatcher_config, uploader, bot, trans).await })
    };
    let t3 = tokio::spawn(async move { trans.start().await });
    let t4 = tokio::spawn(async move { start_onebot(config.clone(), onebot).await });

    if let Some(igneous_task) = igneous_task {
        tokio::try_join!(t1, t2, t3, t4, igneous_task)?;
    } else {
        tokio::try_join!(t1, t2, t3, t4)?;
    }
